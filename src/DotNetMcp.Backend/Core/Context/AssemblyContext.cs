...
public record AssemblyLoadResult
{
    ...
    /// <summary>
    /// 创建远程加载成功结果（无本地 Context）
    /// </summary>
    public static AssemblyLoadResult SuccessRemote(string mvid, string name)
        => new() { IsSuccess = true, RemoteMvid = mvid, RemoteName = name };

    /// <summary>
    /// 远程程序集 MVID（仅用于远程后端）
    /// </summary>
    public string? RemoteMvid { get; init; }
